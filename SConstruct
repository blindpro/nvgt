...
Help("""
	Available custom build switches for NVGT:
		...
		deps=build, download, or unmanaged (default unmanaged)): How to fetch dependencies required to build NVGT? build = use vcpkg to build from source, download = download prebuilt binaries from nvgt.dev if newer than existing, unmanaged = assume dependencies are in place.
